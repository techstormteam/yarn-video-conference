...
$(document).bind('entered.muc', function (event, jid, info, pres) {
    ...
    if (info.isFocus)
    {
        ...
        messageHandler.notify('Happy Yarning!', 'connected', 'connected');
        return;
    }
...
$(document).bind('left.muc', function (event, jid) {
    ...
    {
        console.info("Yarn videochat owner has left the conference");
...
$(document).bind('presence.muc', function (event, jid, info, pres) {
    ...
    if($(pres).find(">bridgeIsDown").length > 0 && !bridgeIsDown) {
        bridgeIsDown = true;
        messageHandler.showError("Error",
            "Yarn Videochat is currently unavailable. Please try again later!");
    }
...
function hangup() {
    ...
    $.prompt("Session Terminated",
        {
            title: "Thanks for using Yarn Videochat! Why not download the FREE app today?",
            persistent: true,
            buttons: {
                "Join the Yarn Videochat again": true
            },
            closeText: '',
